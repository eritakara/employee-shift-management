...
開発中やテスト時には、手軽に動作確認ができる **H2 Database** を使用しています。公開用の本番環境では、クラウド上で安定して利用できる **Supabase PostgreSQL** を使用しています。
...
有休関連のテーブルも将来の機能拡張に向けて用意していますが、ここでは現在の主要機能に関係するテーブルを中心に説明します。

| テーブル名 | 管理している内容 | 主な項目 |
| :--- | :--- | :--- |
| **`users`** | システムを利用する従業員情報 | 社員番号、氏名、メールアドレス、パスワード、権限、所属店舗、部署 |
...
1. **従業員が修正申請を作成したとき**
   ...
   * 同時に、承認できる店長へ通知が作成されます。

2. **店長が承認または却下したとき**
   * 店長が承認または却下を行うと、申請データの状態が更新されます。
