document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-copy-target]').forEach(input => {
    ...
    button.addEventListener('click', () => {
      ...
      if (text) {
        navigator.clipboard.writeText(text).then(() => {
          alert(document.documentElement.lang === 'en' ? 'Copied to clipboard.' : 'コピーしました。');
        }).catch(err => {
...
function translatePage() {
  ...
  const dictionary = {
    ...
    '営業所管理':'Branch offices','部署管理':'Departments','勤務区分・休憩・必要人数管理':'Work types, breaks, and staffing requirements',
    '雇用形態・資格名称管理':'Employment and qualification types','データ出力':'Data export','操作履歴':'Audit log',
    ...
    'すべて既読にする':'Mark all as read','通知はありません。':'No notifications.','表示する通知':'Show','未読のみ':'Unread only','すべて表示':'Show all','既読':'Read','未読の通知はありません。過去の通知は「すべて」から確認できます。':'There are no unread notifications. Past notifications are available under All.','詳細':'Details',
