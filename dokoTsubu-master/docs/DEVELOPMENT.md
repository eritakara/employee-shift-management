...
シフト、有休、勤怠を管理するJava Webアプリケーションの開発者向け詳細マニュアルです。
本ドキュメントでは、ローカル開発環境の構築、ビルド、テスト、および Docker や Render + Supabase (PostgreSQL) へのデプロイ手順について解説します。
...
データベース構造および初期データの詳細については、`database.md` を参照してください。本番データベースが空の状態でアプリを起動すると、テーブル構造および初期シードデータが自動的に構築されます。
...
2. **Settings** タブで、**Branch** を `docs/portfolio-readme` (または検証用ブランチ) に変更し、保存します。
...
* `JDBC_URL` = `jdbc:postgresql://YOUR_SUPABASE_SESSION_POOLER_HOST:5432/postgres?sslmode=require` （SupabaseのSession Poolerへの接続用JDBC URL。ホスト名はご自身のプーラーのホスト名、ポートは `5432` または `6543` などの実際の接続先ポートに置き換えてください。※ユーザー名とパスワードは含めません）
* `DB_USER` = `postgres.YOUR_PROJECT_REF` （接続用のユーザー名。Supabaseのプーラー画面に表示される Connection PoolerのUser、例: `postgres.YOUR_PROJECT_REF` をそのまま指定してください。※通常のpostgresとは異なり、サフィックスが付きます）
* `DB_PASSWORD` = `YOUR_SUPABASE_DB_PASSWORD` （接続用のパスワード。SupabaseのDatabase Passwordを指定してください）
* `INITIAL_HR_EMAIL` = `YOUR_INITIAL_HR_EMAIL` （本番でログインする最初の管理者メールアドレス）
* `INITIAL_HR_PASSWORD` = `YOUR_INITIAL_HR_PASSWORD` （最初の管理者のパスワード。初期値のまま起動しようとすると安全のため起動エラーになります）
* `DEMO_ACCOUNTS_ENABLED` = `true` （ポートフォリオデモ環境用の全拠点デモアカウントを自動生成する場合に `true` を指定）
* `DEMO_ACCOUNTS_PASSWORD` = `YOUR_DEMO_ACCOUNTS_PASSWORD` （自動生成されるデモアカウントの共通パスワード。本番環境で `DEMO_ACCOUNTS_ENABLED=true` の場合は必須）
...
| `JDBC_URL` | `jdbc:postgresql://YOUR_SUPABASE_SESSION_POOLER_HOST:5432/postgres?sslmode=require` | **優先順位 1**: PostgreSQL 接続用 JDBC URL (Session Poolerポート経由推奨)。<br>※`postgres://` または `postgresql://` 形式の接続文字列は、自動的に `jdbc:postgresql://` 形式へ置換されますが、ユーザー名・パスワードは含めません。 |
| `DB_USER` | `postgres.YOUR_PROJECT_REF` | **接続ユーザー**: Supabaseのプーラー設定画面に表示されるユーザー名（例: `postgres.YOUR_PROJECT_REF`）を指定してください。 |
| `DB_PASSWORD` | `YOUR_SUPABASE_DB_PASSWORD` | **接続パスワード**: SupabaseのDatabase Passwordを指定してください。 |
| `INITIAL_HR_EMAIL` | `YOUR_INITIAL_HR_EMAIL` | **初回管理者（HRロール）のメールアドレス**: 本番環境で初期登録される管理者のメールアドレスです。初期ダミーアドレスのまま本番稼働させないために設定します。 |
| `INITIAL_HR_PASSWORD` | `YOUR_INITIAL_HR_PASSWORD` | **初回管理者の初期パスワード**: 本番の安全性を高めるため、半角英数字記号を含む強力なパスワードを指定してください。初期ダミーパスワードのままで本番起動しようとすると、安全のため起動時にエラー（例外）が発生しアプリが停止します。 |
| `DEMO_ACCOUNTS_ENABLED` | `true` | **デモアカウント自動生成フラグ**: ポートフォリオデモ環境用に各拠点（本社＋6支店）に「店長1名・従業員4名」のデモアカウントを自動生成する場合は `true` に設定します。 |
| `DEMO_ACCOUNTS_PASSWORD` | `YOUR_DEMO_ACCOUNTS_PASSWORD` | **デモアカウント共通パスワード**: 自動生成されるデモアカウントに設定する共通パスワードです。本番環境で `DEMO_ACCOUNTS_ENABLED=true` に設定する場合は、セキュリティ保護のためデフォルト値以外の強力なパスワードを指定する必要があります（初期値のままだと起動例外が発生します）。 |
| `RESET_HR_PASSWORD_ENABLED` | `true` / `false` | **HRパスワード一時リセット有効化フラグ**: 本番環境で特定のHRユーザーのパスワードを安全に変更する機能です。再設定を行う場合は `true` に設定します。変更完了後は `false` に戻すか環境変数自体を削除することを推奨します。 |
| `RESET_HR_EMAIL` | `YOUR_RESET_HR_EMAIL` | **パスワード再設定対象のメールアドレス**: パスワードを更新したいアカウントのメールアドレス（例: `hr.demo@example.com`）です。指定されたアドレスが実際に登録されており、ロールが `HR` である必要があります。 |
| `RESET_HR_PASSWORD` | `YOUR_RESET_HR_PASSWORD` | **新規パスワード**: 再設定する新しいパスワードです。セキュリティ保護のため、デフォルト値や空値以外の強力なパスワードを指定する必要があります（初期値のままだと起動例外が発生します）。実パスワードはここには公開せず、個別に管理・設定してください。 |
...
* **Session Pooler (推奨・最優先)**:
  ...
  * 接続文字列の例: `jdbc:postgresql://YOUR_SUPABASE_SESSION_POOLER_HOST:5432/postgres?sslmode=require` (ポートは実際のSession Poolerで指定されているポート番号を指定してください)
  * ※ `JDBC_URL` にはユーザー名・パスワードを含めず、`DB_USER` と `DB_PASSWORD` を個別に設定してください。
* **Direct Connection (非推奨/動作不可の場合あり)**:
  * 接続元（Render等）が IPv6 に完全対応している環境、または有料オプションである **IPv4 Add-on** を適用している場合のみ動作します。ホスト名例（`db.YOUR_PROJECT_REF.supabase.co`）等の Direct Connection は通常のテストデプロイ環境では動作しないため非推奨です。
