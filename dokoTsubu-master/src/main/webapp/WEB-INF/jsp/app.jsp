...
<head>
  ...
  <link rel="stylesheet" href="<%= ctx %>/assets/app.css?v=20260627-2">
</head>
<body>
  ...
  <div class="main-column">
    ...
    <main class="content" id="main-content" tabindex="-1">
      ...
      <% } else if (pageKey.startsWith("shifts/")) { Number selectedShiftBranch=(Number)request.getAttribute("shiftBranchId"); String selectedShiftBranchQuery=selectedShiftBranch==null?"":"&amp;branchId="+selectedShiftBranch.longValue(); boolean printDialogRequested=pageKey.equals("shifts/print")&&"1".equals(request.getParameter("printDialog")); boolean shiftMonthAutoSubmit=List.of("shifts/mine","shifts/request","shifts/change","shifts/manage","shifts/confirm").contains(pageKey); %>
        ...
        <details class="section workflow-editor no-print"><summary>個別に勤務区分を変更</summary><form method="post" class="form-grid"><input type="hidden" name="action" value="saveShift"><input type="hidden" name="returnPage" value="shifts/manage"><label>従業員<select name="userId" required><%for(Map<String,Object> person:people){%><option value="<%=person.get("id")%>"><%=e(person.get("employee_number"))%> <%=e(person.get("name"))%></option><%}%></select></label><label>日付<input type="date" name="date" required></label><label>勤務区分<select name="workType" required><%for(Map<String,Object> wt:workTypes){%><option value="<%=wt.get("code")%>"><%=e(en?wt.get("name_en"):wt.get("name_ja"))%></option><%}%></select></label><label>状態<select name="status"><option value="DRAFT">下書き</option><option value="SUBMITTED">提出済み</option><option value="CONFIRMED">確定</option></select></label><label class="span-2">備考・理由<input type="text" name="note" maxlength="1000"></label><div class="span-all"><button class="primary" type="submit">変更を保存</button></div></form></details>
        <%}%>
...
<script src="<%=ctx%>/assets/app.js?v=20260627-2"></script>
</body>
</html>
