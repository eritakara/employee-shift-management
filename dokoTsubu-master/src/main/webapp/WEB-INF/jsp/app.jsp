...
<%!
  ...
  private String mailStatusLabel(Object value) {
    String status = String.valueOf(value);
    if ("QUEUED".equals(status)) return "送信待ち";
    if ("SENDING".equals(status)) return "送信中";
    if ("SENT".equals(status)) return "送信済み";
    if ("RETRY".equals(status)) return "再送待ち";
    if ("FAILED".equals(status)) return "送信失敗";
    return e(value);
  }
  ...
  private String warningLabel(Object value) {
    String warning = String.valueOf(value);
    if ("STAFF_SHORTAGE".equals(warning)) return "人員不足";
    if ("NIGHT_REST".equals(warning)) return "夜勤明け未休";
    return e(value);
  }
...
<head>
  ...
  <link rel="stylesheet" href="<%= ctx %>/assets/app.css?v=20260627-4">
</head>
<body>
    ...
    <main class="content" id="main-content" tabindex="-1">
      ...
      <% } else if (pageKey.startsWith("shifts/")) { Number selectedShiftBranch=(Number)request.getAttribute("shiftBranchId"); String selectedShiftBranchQuery=selectedShiftBranch==null?"":"&amp;branchId="+selectedShiftBranch.longValue(); boolean printDialogRequested=pageKey.equals("shifts/print")&&"1".equals(request.getParameter("printDialog")); boolean shiftMonthAutoSubmit=List.of("shifts/mine","shifts/request","shifts/change","shifts/manage","shifts/confirm").contains(pageKey); %>
        ...
        <section class="section preference-section"><div class="section-header"><div><h2>希望日をまとめて選択</h2><p class="muted">希望がある日だけ選択してください。未選択日は自動割当の対象になります。</p></div></div>
          ...
          <form method="post" data-preference-form<%=deadlineClosedSubmitted?" data-deadline-closed-message=\""+e(deadlineClosedMessage)+"\" onsubmit=\"alert(this.dataset.deadlineClosedMessage); return false;\"":""%>>
            ...
            <div class="preference-submit"><button class="primary" type="submit" <%=canSubmitPreference?"":"disabled"%>><%=preferenceSubmission.isEmpty()?"提出する":"更新して再提出する"%></button></div>
          </form>
        </section>
...
<script src="<%=ctx%>/assets/app.js?v=20260627-4"></script>
</body>
</html>
