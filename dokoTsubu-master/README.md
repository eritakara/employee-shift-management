...
* `DB_PASSWORD` = `<SupabaseのDatabase Password>` （接続用のパスワード。Supabaseのデータベースパスワードを指定してください）
...
| `JDBC_URL` | `jdbc:postgresql://<Supabase Session pooler Host>:5432/postgres?sslmode=require` | **優先順位 1**: PostgreSQL 接続用 JDBC URL (Session Poolerポート経由推奨)。<br>※`postgres://` または `postgresql://` 形式の接続文字列も、アプリが自動的に `jdbc:postgresql://` 形式へ置換します。ユーザー名・パスワードは含めません。 |
...
| `DB_USER` | `postgres.xxxxxx` | **接続ユーザー**: Supabaseのプーラー設定画面に表示されるユーザー名（例: `postgres.xxxxxx`）を指定してください。 |
| `DB_PASSWORD` | `<SupabaseのDatabase Password>` | **接続パスワード**: データベースのパスワードを指定してください。 |
...
* **Direct Connection (非推奨/動作不可の場合あり)**:
  * 接続元（Render等）が IPv6 に完全対応している環境、または有料オプションである **IPv4 Add-on** を適用している場合のみ動作します。通常のテストデプロイ環境では動作しないため非推奨です。
