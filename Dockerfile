...
CMD ["sh", "-c", "mkdir -p \"${SHIFTFLOW_DATA_DIR:-/opt/shiftflow/data}\" && sed -i \"s/port=\\\"8080\\\" protocol=\\\"HTTP\\/1.1\\\"/port=\\\"${PORT:-10000}\\\" protocol=\\\"HTTP\\/1.1\\\"/\" \"$CATALINA_HOME/conf/server.xml\" && sed -i \"s/port=\\\"8005\\\" shutdown=\\\"SHUTDOWN\\\"/port=\\\"-1\\\" shutdown=\\\"SHUTDOWN\\\"/\" \"$CATALINA_HOME/conf/server.xml\" && sed -i \"s#<Engine name=\\\"Catalina\\\" defaultHost=\\\"localhost\\\">#<Engine name=\\\"Catalina\\\" defaultHost=\\\"localhost\\\"><Valve className=\\\"org.apache.catalina.valves.RemoteIpValve\\\" remoteIpHeader=\\\"x-forwarded-for\\\" protocolHeader=\\\"x-forwarded-proto\\\" />#\" \"$CATALINA_HOME/conf/server.xml\" && exec catalina.sh run"]
