...
* **Direct Connection (推奨・最優先)**:
  ...
  * ※ **IPv6接続に関する注意点**: Supabase の Direct Connection (ポート `5432`) は、接続元のネットワーク環境（Render等）の IPv6 対応状況によって影響を受ける場合があります。もし Render 上でのデプロイ起動時に `connection timeout` や `connection refused` などの接続エラーが発生した場合は、後述の **Session Pooler (ポート `6543`)** への切り替え、または Supabase の有料オプションである **IPv4 Add-on** の適用を検討してください。
