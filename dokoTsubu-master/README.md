...
> * ローカル開発環境用のデモアカウント（`hr@example.com` 等）は、本番環境 (`APP_ENV=production`) では自動生成されず、上記で設定した初期管理者アカウントのみが使用可能となります。

> [!IMPORTANT]
> **Render無料プラン（Free Plan）についてのご注意**:
> Render Free Plan を利用しているため、一定時間アクセスがない場合はサービスがスリープします。初回アクセス時は起動まで30〜60秒程度かかる場合があります。画面が表示されるまでブラウザを閉じずにお待ちください。一度起動すれば、その後は通常の速度で動作します。
