...
<head>
  ...
  <link rel="stylesheet" href="<%= ctx %>/assets/app.css?v=20260626-3">
</head>
<body>
      ...
      <% } else if (pageKey.startsWith("shifts/")) { Number selectedShiftBranch=(Number)request.getAttribute("shiftBranchId"); String selectedShiftBranchQuery=selectedShiftBranch==null?"":"&amp;branchId="+selectedShiftBranch.longValue(); boolean printDialogRequested=pageKey.equals("shifts/print")&&"1".equals(request.getParameter("printDialog")); boolean shiftMonthAutoSubmit=List.of("shifts/mine","shifts/request","shifts/change","shifts/manage","shifts/confirm").contains(pageKey); %>
        ...
        <section class="section no-print"><div class="section-header"><h2><%=pageKey.equals("shifts/change")?"変更・休みを申請":"勤務区分を登録"%></h2></div>
          <form method="post" class="form-grid">
            ...
            <label>勤務区分<select name="workType" required><% for(Map<String,Object> wt:workTypes){ if(pageKey.equals("shifts/change") && "LEAVE".equals(String.valueOf(wt.get("code")))) continue; %><option value="<%=wt.get("code")%>"><%=e(en?wt.get("name_en"):wt.get("name_ja"))%></option><% } %></select></label>
        ...
        <% if(pageKey.equals("shifts/mine") || pageKey.equals("shifts/print")){ %>
          <% { String rosterTitle="シフト"; String rosterLink=null; List<Map<String,Object>> rosterBranches=(List<Map<String,Object>>)request.getAttribute("shiftBranches"); Long rosterBranchId=selectedShiftBranch==null?user.getBranchId():selectedShiftBranch.longValue(); %>
            <%@ include file="_shiftRoster.jspf" %>
          <% } %>
        <% } else if (!pageKey.equals("shifts/manage")) { %>
