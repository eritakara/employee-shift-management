...
- [要件定義書](../シフト・有休管理アプリ_要件定義書.md)
- [タスクリスト](../シフト・有休管理アプリ_タスクリスト.md)
- [技術構成メモ](../シフト・有休管理アプリ_技術構成メモ.md)
- [面接説明用要約](../シフト・有休管理アプリ_面接説明用要約.md)
...
| `RESET_HR_EMAIL` | `<パスワードを変更したいHRユーザーのメール>` | **パスワード再設定対象のメールアドレス**: パスワードを更新したいアカウントのメールアドレス（例: `hr.demo@example.com`）です。指定されたアドレスが実際に登録されており、ロールが `HR` である必要があります。 |
| `RESET_HR_PASSWORD` | `<強力な新規パスワード>` | **新規パスワード**: 再設定する新しいパスワードです。セキュリティ保護のため、デフォルト値（`Password1!`）や空値以外の強力なパスワードを指定する必要があります（初期値のままだと起動例外が発生します）。実パスワードはここには公開せず、個別に管理・設定してください。 |
